...
`llreview review-gap-stamp-pump` は、`review-gap-examples.jsonl` のうち human-gate 待ちの teacher/external gap を、短い根拠、local finding/watch との距離、ハンコ補助、`external-verdict` command 付きで `out/review-history/review-gap-stamp-pump/` にまとめます。既定では read-only です。連続でハンコを押す場合は TTY で `llreview review-gap-stamp-pump --stamp` を使い、`y` valid、`f` not actionable、`c` covered、`n` unsure、`s` skip、`q` quit を入力します。`y` は diff-local かつ actionable と判断できる時だけ使います。
...
`llreview specbackfill-overlap --run <run-id>` は、保存済み `review_items(source='specbackfill')` を同じ scope の local model `review_items` / imported `external_items` と deterministic に照合する preview です。`--specbackfill-json` を渡した場合だけ JSON findings を override 入力として使えます。external missed by local、external covered by specbackfill、model/specbackfill overlap、specbackfill false-positive verdict を artifact に出しますが、外部 item が false-positive / not-actionable と判定済みなら missed/covered signal からは外します。artifact には将来の `item_links` 書き込み境界を確認する link readiness も出しますが、現段階の判断は preview-only 継続です。既定では DB writes、GitHub API、PR checkout、PR code execution、PR comment 投稿は行わず、raw body / raw evidence / raw diff も出しません。preview artifact の path/sha256 だけを review DB に残す場合は、明示 opt-in の `--record-db-artifacts` を `--run` と一緒に使います。この flag でも `item_links` / `item_verdicts` は書きません。

将来 `specbackfill-overlap` から `item_links` を保存する apply path を追加する場合も、既定は preview-only / no DB writes のままにし、保存は別 command か明示 opt-in flag に限定します。apply は `--run` で保存済み `review_items(source='specbackfill')` に anchor できる候補だけを対象にし、false-positive / not-actionable / out-of-scope 判定済みの外部 item は除外します。既存の `(review_item_id, external_item_id)` pair があれば relation が違っても新規 link は作りません。relation は matcher の relation を再利用し、`source=specbackfill`、rule id、score、match relation は note 側に残します。specbackfill 由来の link は local/model coverage として数えず、`item_verdicts` の自動書き込みもしません。
