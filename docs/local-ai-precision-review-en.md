...
`q` to quit. For prompt/rule candidates, press `y` to approve an active DB
calibration, `v` to view the preview, or `s` to skip. It hides full body text
and body digests by default; use `--verbose` or `--include-active` only when you
want the longer audit view. Use `llreview learn-review --dry-run` to preview the
queue.
