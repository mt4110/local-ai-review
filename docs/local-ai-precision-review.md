...
`llreview specbackfill-overlap --run <run-id>` は、保存済み `review_items(source='specbackfill')` と、既存DB内の local `review_items` / imported `external_items` の overlap を preview します。`--specbackfill-json specbackfill.json` を渡した場合だけ、`specbackfill check --format json --fail-on off` の findings を override 入力として使えます。既定では local model finding だけを候補にし、external missed by local、external covered by specbackfill、model/specbackfill overlap、specbackfill false-positive verdict を集計します。false-positive / not-actionable 判定済みの外部 item は missed/covered signal から外します。将来 `item_links` に書けるかの link readiness も artifact に出しますが、現段階では preview-only で、specbackfill link を local/model coverage として扱わないことを明示します。DB writes、GitHub API、PR checkout、PR code execution、PR comment 投稿は行いません。`--record-db-artifacts` を明示した場合だけ artifact path / sha256 を `artifacts` に保存しますが、この場合も `item_links` / `item_verdicts` は書きません。結果は `out/review-history/specbackfill-overlap/` の Markdown/JSON artifact に保存され、raw body、raw evidence、raw diff ではなく id、path、line、rule id、digest だけを出します。確認だけなら `--dry-run` を使います。

将来 `specbackfill-overlap` から `item_links` を保存する apply path を足す場合も、既定は preview-only / no DB writes のままにし、保存は別 command か明示 opt-in flag に限定します。apply は `--run` で保存済み `review_items(source='specbackfill')` に anchor できる候補だけを対象にし、false-positive / not-actionable / out-of-scope 判定済みの外部 item は除外します。既存の `(review_item_id, external_item_id)` pair があれば relation が違っても新規 link は作りません。relation は matcher の relation を再利用し、`source=specbackfill`、rule id、score、match relation は note 側に残します。specbackfill 由来の link は local/model coverage として数えず、`item_verdicts` の自動書き込みもしません。
